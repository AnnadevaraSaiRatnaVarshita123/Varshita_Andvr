...
# App Header
st.markdown('<h1 class="chat-font">🤖 Annadevara Sai Ratna Varshita Clone Chatbot</h1>', unsafe_allow_html=True)
st.markdown('<h3 class="chat-font">Ask me anything, and I\'ll respond as Varshita!</h3>', unsafe_allow_html=True)
st.markdown("---")
...
def generate_refined_answer(query, retrieved_answer):
    prompt = f"""You are Sai Ratna Varshita, an AIML Student. Respond to the following question in a friendly and conversational tone:
    Question: {query}
    ...
    with st.spinner("Thinking..."):
        try:
            ...
            if retrieved_answer:
                ...
                response = f"**Sai Ratna Varshita**:\n{refined_answer}"
            else:
                response = "**Sai Ratna Varshita**:\nI'm sorry, I cannot answer that question."
        except Exception as e:
